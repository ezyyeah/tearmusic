...
class _AlbumViewState extends State<AlbumView> {
  ...
  CachedItem<List<MusicTrack>?>? trackResults;
  ...
  void initState() {
    ...
    context.read<MusicInfoProvider>().albumTracks(widget.album.id).listen((value) {
      trackResults = value;
      setState(() {});
    });
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      ...
                                      Selector<UserProvider, List<String>>(
                                        selector: (_, p) => p.library?.liked_albums ?? [],
                                        ...
                                        builder: (context, data, _) {
                                          return LikeButton(
                                            ...
                                            isLiked: data.contains(widget.album.id),
                                            onTap: (isLiked) async {
                                              if (!isLiked) {
                                                context.read<UserProvider>().putLibrary(widget.album, LibraryType.liked_albums);
                                              } else {
                                                context.read<UserProvider>().removeFromLibrary(widget.album, LibraryType.liked_albums);
                                              }
                                              return !isLiked;
                                            },
                        ...
                        SliverList(
                          delegate: SliverChildBuilderDelegate(
                            (BuildContext context, int index) =>
                                CachedOpacity(type: trackResults?.type, child: AlbumTrackTile(trackResults!.item![index])),
                            childCount: trackResults!.item!.length,
                          ),
                        ),
